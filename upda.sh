...
# 检查是否有互联网连接
if ! ping -t 1 -W 1 github.com > /dev/null; then
...
wget "${REPO_URL}/releases/download/${latest_release}/openwrt-rockchip-armv8-friendlyarm_nanopi-r2s-ext4-sysupgrade.img.gz" || exit 1
...
# 上传固件
echo "正在上传固件"
scp -r ~/Downloads/update/openwrt-rockchip-armv8-friendlyarm_nanopi-r2s-ext4-sysupgrade.img.gz root@${ROUTER_IP}:/tmp/tmp/
# 执行更新
echo "正在执行更新,请耐心等待"
ssh root@${ROUTER_IP} "sysupgrade -v /tmp/tmp/openwrt-rockchip-armv8-friendlyarm_nanopi-r2s-ext4-sysupgrade.img.gz"
